...
import * as figtersRepository from "../Repositories/figtersRepository";

export async function getAllFigters() {
  const valuesFigters = await figtersRepository.allFigters();
  return valuesFigters;
}

export async function compareStars(firstUser: string, secondUser: string) {
...
async function getFighter(username: string) {
  const fighter = await figtersRepository.findByUsername(username);

  if (!fighter) {
    const createdFighter = await figtersRepository.insert(username);
...
async function getBattleResult(
  ...
  console.log(secondFighter.username);
...
async function updateWinnerAndLoserStats(winnerId: number, loserId: number) {
  await figtersRepository.updateStats(winnerId, "wins");
  await figtersRepository.updateStats(loserId, "losses");
}
...
async function updateDrawStats(
  ...
  await figtersRepository.updateStats(firstFighterId, "draws");
  await figtersRepository.updateStats(secondFighterId, "draws");
}
